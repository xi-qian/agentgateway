...
Configuration:
    client_id: DingTalk app key (or DINGTALK_CLIENT_ID env var)
    client_secret: DingTalk app secret (or DINGTALK_CLIENT_SECRET env var)
"""
...
class DingTalkAdapter(PlatformAdapter):
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self._client_id = self._config.get("client_id") or os.getenv("DINGTALK_CLIENT_ID", "")
        self._client_secret = self._config.get("client_secret") or os.getenv("DINGTALK_CLIENT_SECRET", "")
    ...
    async def send_response(self, group_id: str, text: str) -> None:
        ...
        if not session_webhook:
            logger.error("[%s] No session_webhook for %s", self.name, chat_id)
            return

        if not self._http_client:
            logger.error("[%s] HTTP client not initialized", self.name)
            return

        payload = {
            "msgtype": "markdown",
            "markdown": {"title": "AgentGateway", "text": text[:self.MAX_MESSAGE_LENGTH]},
        }
        ...
        try:
            ...
            logger.error("[%s] Timeout sending message to DingTalk", self.name)
        except Exception as e:
